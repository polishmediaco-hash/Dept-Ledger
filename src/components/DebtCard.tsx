      ...
      <div className="p-4 flex flex-col justify-between">
        <div>
            ...
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              ...
              {showMenu && (
                <>
                  ...
                  />
                  <div className="absolute right-0 top-8 z-50 w-44 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-xl py-1.5 animate-in fade-in zoom-in-95">
                    {!isSettled ? (
                      <button
                        type="button"
                        onClick={() => {
          ...
          <div className="flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
            ...
            {/* Fast Settle action on card */}
            <div onClick={(e) => e.stopPropagation()}>
              {!isSettled ? (
                <button
                  type="button"
                  onClick={() => onRecordPayment(debt)}
                  className="py-1.5 px-3 bg-zinc-900 dark:bg-zinc-100 active:bg-zinc-800 dark:active:bg-zinc-200 hover:bg-zinc-800 dark:hover:bg-white text-white dark:text-zinc-900 font-semibold rounded-lg text-[11px] flex items-center gap-1.5 transition-all active:scale-95 shadow-2xs cursor-pointer"
                >
                  <Plus className="w-3 h-3 text-zinc-300 dark:text-zinc-700" />
                  <span>Settle</span>
                </button>
              ) : (
                <span className="text-[10px] text-emerald-700 dark:text-emerald-400 font-bold">Settled</span>
              )}
            </div>
          </div>
        </div>

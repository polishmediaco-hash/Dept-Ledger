      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200/90 dark:border-zinc-800 rounded-2xl p-4 shadow-2xs transition-colors">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-zinc-500 dark:text-zinc-400 font-medium">{allPayments.length} payments and settlements recorded</p>
          </div>
        </div>
      ...
      <div className="space-y-2.5">
        {filteredPayments.length === 0 ? (
          ...
          filteredPayments.map((payment) => {
            const isReceivable = payment.direction === 'owed_to_me';
            return (
              <div 
                key={payment.id} 
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3 min-w-0">
                    <div className={`mt-0.5 h-7 w-7 rounded-lg flex items-center justify-center shrink-0 border font-bold text-xs tabular-nums ${
                      isReceivable 
                        ? 'theme-rec-badge' 
                        : 'theme-pay-badge'
                    }`}>
                    ...
                    <div className="min-w-0 space-y-0.5">
                      <h3 className="text-xs font-bold text-zinc-900 dark:text-zinc-100 leading-none truncate">{payment.contactName}</h3>
                      <p className="text-[11px] text-zinc-500 dark:text-zinc-400 font-medium truncate max-w-[170px]">{payment.debtTitle}</p>
                  ...
                  <div className="text-right shrink-0">
                    <div className={`text-xs font-bold tabular-nums ${
                      isReceivable ? 'theme-rec-text' : 'theme-pay-text'
                    }`}>
                      {isReceivable ? '+' : '−'}{payment.amount.toLocaleString()} {currency}
                    </div>
                    {payment.note && (
                      <p className="text-[10px] text-zinc-400 dark:text-zinc-500 italic mt-0.5 max-w-[140px] truncate">"{payment.note}"</p>
                    )}
                  </div>
                </div>
                ...
                <div className={`absolute top-0 right-0 w-1 h-full ${
                  isReceivable ? 'theme-rec-bg' : 'theme-pay-bg'
                }`} />
              </div>
            );
          })
